...
\begin{itemize}
    \item Mining Time (Seconds): 3
    \item Number of Transactions to Mine: 1
    \item Maximum Number of Pending Transactions: 10
\end{itemize}

For the parameters of \textit{Minimum Value of Transactions}, we set 1 for Alice, 3 for Bob, and 8 for Charlie. Hence, Alice is expected to mine blocks faster than Bob and Charlie. For the delays of networks, we set all the parameters to zero.
...
After finishing the configuration, the user can see the initial status of the blockchain system like Figure \ref{fig:initial status}. 
